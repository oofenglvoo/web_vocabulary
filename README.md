...
- **详情页查词典**: 单词详情页右上角「查词典」调用必应词典，就地展开音标、词性释义、网络释义与双语例句；面板上方提供「覆盖本地翻译」，一键把词典释义、音标、例句写回词条。释义按中文分号**拆成独立义项**（同词性可多条，共用词性标签）；例句写入独立的 `dictionaryExamples` 字段，详情页**多行逐条展示**（含中文翻译与来源），首条同时写入旧的单数字段保持兼容；完整备份与 JSON / CSV 导出均包含全部释义与例句。本地已有的英文释义按位置保留，不会被清空
- **详情页自动翻译**: 单词详情页右上角提供「自动翻译」开关（全站持久，存于 localStorage 并随备份导出/恢复）。开启后，打开**英语**单词详情页会自动调用在线翻译（Google → MyMemory 回退，结果缓存 30 天）并在词头下方展示译文与来源；仅展示不写库，日语词不自动翻译，「查词典」按钮行为不受影响
- **快速翻译**: 首页提供独立搜索框，输入英语或日语单词、短语或句子后跳转到翻译页，自动识别语言并翻译成中文；同时按输入语言查询对应本地词库，命中时展示本地释义、例句、笔记和分类
- **在线词典**: 首页搜索框「查词」或首页「在线词典」入口进入 `/dictionary`，单个英文单词可查美/英音标、词性释义、词形变化、网络释义与带来源标注的双语例句（例句中高亮目标词）；数据来自必应词典，结果缓存 30 天
- **翻译页查词典**: 在 `/translate` 翻译单个英文单词后，结果卡片右上角出现「查词典」按钮，点击就地展开同一张必应词典卡片，无需跳转页面；输入句子或日语时不显示该入口
...
- **日语重复词条**: 批量导入时可选择跳过重复词条；取消跳过后允许保存同一表记的多条独立记录，搜索会显示全部结果
...
- **日语重复词条**: 日语导入可选择是否跳过已存在词条；取消跳过后允许同一表记保存多条独立记录，词库搜索会显示全部匹配结果
- **分类导出**: 分类详情页支持导出当前单词为 JSON 或 CSV，包含原有释义、例句、词典批量例句（dictionaryExamples）、笔记、在线翻译及翻译来源字段
...
src/
  ...
  utils/          # 工具函数 (srs 7周期算法, tts, dictionary 在线词典（必应）, translation 在线翻译多通道, translationPrefs 自动翻译偏好, import, export, search, achievements, studyPrefs, categoryType)
```
