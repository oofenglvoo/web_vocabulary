...
- **日语词库**: 表记 + 假名读音（不使用罗马字）、音调（⓪/①/⓪①）、多释义（词性/日文释义/中文翻译）、例句三件套（例句/例句读音/例句翻译）
...
- **预学习流程**: 开始学习后先查看当天词条；可点击词条进入详情并按顺序浏览，到最后一个词条时可点击「开始测试」，确认后进入所选题型
- **复习流程**: 复习入口跳过预学习列表，直接进入测试；复习完成后按当前 SRS 状态动态产生到期词条
- **学习状态**: 当天确认开始测试后，再次点击「学习」直接进入测试；次日自动清除该确认状态，不清空学习记录和 SRS 数据
- **中断恢复**: 普通测验保存当天的队列与当前位置；快速自测保存未提交的评分，再次点击「学习」可继续中断前的进度，完成后自动清除临时状态
- **学习卡片显示**: 长单词和长短语在回忆式、选择题、快速自测中保持完整显示；评分期间防止重复提交，模糊/忘记后的词条可正常再次评分
...
- 每日新词配额，各计划进度独立（同一分类多计划互不串味）；复习数量按 SRS 动态产生，不单独设置每日复习词数
- **今日新词学满后**点「学习」弹确认框，可选择"额外学习"再学一轮（不计入今日配额，仅计入 startedIds）
- **今日新词统计**显示配额内 + 额外学习的总数（如 20/10）
- 复习队列动态加载当前计划内全部到期且未掌握的词条，数量由 SRS 状态决定
...
### 其他
- **语音朗读**: 英语优先有道真人发音、日语优先 Google 翻译语音（自动回退浏览器 Web Speech 合成，自动挑选 Natural/Google 音色）；自动检测日语（假名 → ja-JP 语音）
- **暗色模式**: 跟随系统或手动切换
- **滚动位置记忆**: 从列表进入详情后返回，自动恢复之前的滚动位置（按路由记录，等待列表渲染完成后精确还原；前进进入新页面回到顶部）
- **批量导入/导出**: JSON / CSV / Text 多格式，支持多释义（definitions 数组）；日语支持 JSON 导入（必填 word/reading，释义用 meaning，可选 accent 音调/partOfSpeech/example/exampleTranslation/notes，兼容旧 expression/kana/definitions 别名，自动创建缺失的日语分类）；完整备份包含全部语言数据
- **分类导出**: 分类详情页支持导出当前单词为 JSON 或 CSV，包含原有释义、在线翻译及翻译来源字段
- **导入反馈**: 批量导入提示实际新增数量，并单独显示因重复而跳过的词条数量
- **列表快速定位**: 单词、短语、收藏夹、分类和计划列表提供回到顶部与到底部按钮，长列表会先完成增量加载再定位
- **收藏夹**: 多目录收藏、默认收藏到上次目录、成功提示可跳转其他目录（英语/短句与日语各自独立一套收藏夹）
- **搜索**: 子串 + 拼音首字母 + 模糊匹配（日语按表记/假名/词性/音调/笔记/释义检索）
- **笔记**: 日语词支持笔记字段——词库卡片直接展示；学习翻面与词条详情页可现场查看/编辑（学习过程中无需中断即可记录释义笔记）
...
# 运行 E2E 测试（98 用例，含语言切换/数据隔离/滚动恢复回归）
npm run test:e2e
